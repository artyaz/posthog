...
def detect_viewset_modules(backend_dir: Path) -> list[Path]:
    """Backend-root modules defining ViewSet subclasses — the move candidates."""
    found = []
...
def build_scan_report(name: str, repo_root: Path = REPO_ROOT) -> dict:
    ...
    view_modules = detect_viewset_modules(backend_dir / "backend")
    return {
        "product": name,
...
def render_scan_report(report: dict) -> str:
    ...
    if report["view_modules"]:
        ...
        lines.append("- none detected at backend root")
    return "\n".join(lines)
...
def build_move_plan(name: str, views: list[str] | None = None) -> MovePlan:
    ...
    if views:
        ...
        view_paths = detect_viewset_modules(backend_dir)
    target_dir = backend_dir / "presentation" / "views"
    view_moves = [(p, target_dir / p.name) for p in view_paths]
    renames = {
        f"products.{name}.backend.{p.stem}": f"products.{name}.backend.presentation.views.{p.stem}" for p in view_paths
    }
    tasks_py = backend_dir / "tasks.py"
    tasks_move = (tasks_py, backend_dir / "tasks" / "tasks.py") if tasks_py.is_file() else None
    return MovePlan(product=name, view_moves=view_moves, module_renames=renames, tasks_move=tasks_move)
