...
def _neutralize_tags(text: str) -> str:
    """Replace angle brackets so an untrusted snippet can't forge the fence's tag boundaries."""
...
def _as_untrusted_data(label: str, lines: list[str]) -> str:
    ...
    recording could embed text that reads as instructions. Wrapping it in a labelled block with an explicit
    "data, not instructions" preamble is the indirect-prompt-injection mitigation used elsewhere in the app
    (see annotations / exports). Callers must `_neutralize_tags` the untrusted parts of each line first.
    """
    body = "\n".join(lines)
    return (
    ...
    def _fetch_and_format(self, scanner_id: str) -> tuple[str, dict[str, Any]]:
        ...
        for scanner_result, created_at in observations:
            ...
            if not isinstance(summary, str) or not summary.strip():
                continue
            title_raw = output.get("title") if isinstance(output.get("title"), str) else None
            # Summary + title are model output over user session content — neutralize tags before fencing.
            clean = _neutralize_tags(_EVENT_ID_CITATION_RE.sub("", summary).strip())
            title = _neutralize_tags(title_raw) if title_raw else None
...
class SearchReplayVisionObservationsTool(MaxTool):
    ...
    async def _arun_impl(
        ...
        try:
            ...
            # Generic content — Max may relay it to the user, so don't surface the raw exception.
            return "Something went wrong searching the observations. Please try again.", {
                "error": "search_failed",
                "details": str(e),
            }
    ...
    def _search(
        ...
        capped_limit = min(limit or DEFAULT_SEARCH_LIMIT, MAX_SEARCH_LIMIT)
        empty = (f"No recordings from {scope_label} matched that search yet.", {"result_count": 0})

        try:
            embedding_response = generate_embedding(self._team, query, model=SEARCH_EMBEDDING_MODEL.value)
        except Exception:
            # Most commonly the org hasn't opted into AI data processing (the worker 403s).
            return (
                "Couldn't run the search — the embedding service is unavailable. This often means the "
                "organization hasn't enabled AI data processing (Settings > AI).",
    ...
    def _resolve_scanner_scope(self, scanner_id: str | None) -> tuple[list[str], str, bool] | None:
        ...
        if scanner_id:
            ...
            if scanner is None or not self.user_access_control.check_access_level_for_object(scanner, "viewer"):
                return None
            return [str(scanner.id)], f'the "{scanner.name}" scanner', False
    ...
    def _format_line(self, obs: ReplayObservation, output: dict[str, Any], *, show_scanner: bool) -> str:
        ...
        # The reasoning, descriptor (freeform tags), and scanner name are all model/user content the line fences
        # as untrusted — neutralize tags so a snippet can't break out of the surrounding <observations> block.
        clean = _neutralize_tags(_EVENT_ID_CITATION_RE.sub("", explanation).strip()[:_SEARCH_SNIPPET_LIMIT])

        prefix = f"{obs.created_at:%Y-%m-%d}"
        # session_id is client-settable, so it's untrusted too — neutralize it like the rest of the line.
        session = _neutralize_tags(str(obs.session_id))
        # In a cross-scanner search, name the scanner each match came from.
        scanner_part = f" {_neutralize_tags(obs.scanner.name)}" if show_scanner and obs.scanner else ""
        descriptor_part = f" [{_neutralize_tags(descriptor)}]" if descriptor else ""
