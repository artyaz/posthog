...
Legacy rows are renamed in place (`users` → `public.users`) and tagged `s3_folder_path=<original>`
...
def _qualify_legacy_row(
    ...
    *,
    ...
    """Rename `row` to qualified form, stash `s3_folder_path`, optionally drop a duplicate.
    ...
    if not row.s3_folder_path:
        row.s3_folder_path = storage_key
        update_fields.append("s3_folder_path")
